...
def customer_create():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash('Вы успешно добавили покупателя!', category='success')
...
def customer_detail(customer_id):
    ...
    if customer:
        ...
        flash('Такого покупателя нет', category='danger')
        return redirect(url_for('index'))
    ...
    if customer:
        ...
        if customer.user_id == current_user.id:
            if request.method == 'POST':
                ...
                db.session.commit()
                flash('Вы успешно удалили покупателя!', category='success')
                return redirect(url_for('index'))
            else:
                return render_template('customer_delete.html', form=form, customer=customer)
        else:
        ...
        if customer.user_id == current_user.id:
            ...
            if request.method == 'POST':
                if form.validate_on_submit():
                    ...
                    db.session.commit()
                    flash('Покупка обновлена', category='success')
                    return redirect(url_for('index'))
                if form.errors:
                    for errors in form.errors.values():
                        for error in errors:
            ...
            flash('Покупатель не найден', category='danger')
            return redirect(url_for('index.html'))
